import { Mic, RotateCcw, Volume2, Play, NotebookPen } from "lucide-react";
...
const MyNoteStudyCard = ({
  ...
  return (
    <section className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm sm:p-5 lg:p-6">
      ...
      <div className="rounded-[20px] border border-dashed border-emerald-200 bg-emerald-50/40 p-5 sm:p-6">
        <div className="flex min-h-[260px] flex-col items-center justify-center text-center">
          <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-600 shadow-sm">
            <NotebookPen size={32} />
          </div>

          <p className="text-sm font-semibold text-emerald-600">학습 문장</p>

          <div className="mt-3 min-h-[56px] max-w-xl text-pretty px-2 text-lg font-bold leading-8 text-slate-800 sm:text-[22px]">
            {selectedSentence ?? "목록에서 학습할 문장을 선택해주세요."}
          </div>

          <p className="mt-3 text-sm leading-6 text-slate-500">
            선택한 문장을 듣고 천천히 따라 말해보세요.
          </p>
          ...
          <div className="mt-6 flex w-full max-w-md flex-col gap-3 sm:flex-row">
            ...
            <button
              ...
              onClick={onRecord}
              disabled={!isSentenceSelected}
              className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-white transition cursor-pointer hover:bg-emerald-600 disabled:cursor-not-allowed disabled:bg-slate-300"
            >
              {hasRecording ? <RotateCcw size={18} /> : <Mic size={18} />}
              {isRecording ? "녹음 중..." : hasRecording ? "재녹음" : "녹음"}
            </button>
